...
let login = (req, res, cb) => {
    ...
    validateLogin.then(resolve => {
        let user = {
            id: resolve.id,
            name: resolve.name,
            username: resolve.username
        }
        let token = jwt.sign({ user }, config.jwt_secret, { expiresIn: 30 });
        let refresh_token = jwt.sign({ user }, config.jwt_secret, { expiresIn: 120 });
        return cb(null, [token, refresh_token]);
    }).catch(reject => {
        if (typeof (reject) === "string") return res.status(401).json({
